/*description:
copy from https://github.com/hmisty/coding/blob/master/blockchain/bitcoin/testhd.js
可以从 自己设置的复杂密码puzzle 生成助记词（24词，256 bits强度，应该可以导入硬件钱包比如ledger nano），然后衍生HD钱包地址，同时支持BTC和ETH地址。
如果不信任第三方软硬件钱包，或者自己想实现HD脑钱包，可以参考并自己生成冷地址。
生成的助记词可以导入metamask（ETH），私钥可以导入electrum（BTC），实现后续动币操作。
*/
